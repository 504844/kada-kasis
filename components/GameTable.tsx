              ...
              >
                ...
                <td className="p-4 text-right w-[30%] align-middle">
                   <div className="flex items-center justify-end gap-3">
                      <div className="flex items-center gap-2">
                         <button 
                            onClick={(e) => { e.stopPropagation(); onToggleFavoriteTeam(game.homeTeam.name); }}
                            className={`opacity-0 group-hover:opacity-100 transition-opacity ${isHomeFavorite ? 'opacity-100' : ''}`}
                         >
                            <Star size={10} className={isHomeFavorite ? "fill-white text-white" : "text-zinc-600 hover:text-white"} />
                         </button>
                         <span 
                            onClick={() => onFilterByTeam(game.homeTeam.name)}
                            className={`font-bold cursor-pointer hover:underline truncate ${isHomeWinner ? 'text-white' : 'text-zinc-400'}`}
                         >
                            {game.homeTeam.name}
                         </span>
                      </div>
                ...
                <td className="p-4 text-center w-[100px] align-middle">
                   <div className="flex items-center justify-center font-mono font-bold text-lg tracking-tighter">
                      <span className={game.status === 'live' ? 'text-orange-500' : isHomeWinner ? 'text-white' : 'text-zinc-500'}>{game.homeScore}</span>
                      <span className="text-zinc-700 mx-1">-</span>
                      <span className={game.status === 'live' ? 'text-orange-500' : isAwayWinner ? 'text-white' : 'text-zinc-500'}>{game.awayScore}</span>
                   </div>
                </td>
                   ...
                   <div className="flex items-center justify-start gap-3">
                      ...
                      <div className="flex items-center gap-2">
                         <span 
                            ...
                            className={`font-bold cursor-pointer hover:underline truncate ${isAwayWinner ? 'text-white' : 'text-zinc-400'}`}
                         >
                            {game.awayTeam.name}
                         </span>
                         <button 
                            onClick={(e) => { e.stopPropagation(); onToggleFavoriteTeam(game.awayTeam.name); }}
                            className={`opacity-0 group-hover:opacity-100 transition-opacity ${isAwayFavorite ? 'opacity-100' : ''}`}
                         >
                            <Star size={10} className={isAwayFavorite ? "fill-white text-white" : "text-zinc-600 hover:text-white"} />
                         </button>
                      </div>
                   </div>
                </td>
